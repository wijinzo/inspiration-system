...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadHistory() {
        ...
        try {
            ...
            historyList.innerHTML = data.history.map((item, idx) => {
                let hooksPreview = '';
                try {
                    const hooks = JSON.parse(item.hooks_json || '[]');
                    if (hooks.length > 0) hooksPreview = hooks[0].substring(0, 60) + '...';
                } catch(e) {}
                return `
                <div class="history-item" onclick="viewHistoryItem(${idx})" style="cursor:pointer">
                    ...
                    ${hooksPreview ? `<div style="font-size:0.75rem;color:var(--text-muted);margin-top:0.3rem;opacity:0.7;"><i class="fa-solid fa-lightbulb"></i> ${hooksPreview}</div>` : ''}
                </div>
            `}).join('');
    ...
    window.viewHistoryItem = function(idx) {
        if (!window._historyCache || !window._historyCache[idx]) return;
        const item = window._historyCache[idx];
    ...
    async function doGenerateScript() {
        // 優先用使用者手動選的科學文獻，若無則 fallback 到 Surprise Me 產生的 matched_science
        const scienceUrl = selectedState.science?.id
            || (currentData && currentData.matched_science ? currentData.matched_science.url : null);

        if (!scienceUrl) {
            alert("請先選擇一筆科學文獻，或使用 Surprise Me 生成！");
            return;
        }
        ...
        try {
            const socialUrl = selectedState.spice && selectedState.spice.type === 'social'
                ? selectedState.spice.id
                : (currentData && currentData.matched_social ? currentData.matched_social.url : null);
            const payload = {
                science_url: scienceUrl,
   ...
   ═══════════════════════════════════════════ */
    ...
    function renderDbTable(type, items) {
        ...
        const apiTypeMap = { science: 'science', social: 'social', trends: 'trend' };
        ...
        tbody.innerHTML = items.map((item, i) => {
            ...
            const delBtn = item.url

                ? `<button class="db-delete-btn" title="刪除" onclick="deleteDbItem('${apiType}','${encodedUrl}',this)"><i class="fa-solid fa-trash-can"></i></button>`
    ...
    window.deleteDbItem = async function(apiType, encodedUrl, btnEl) {

        const url = decodeURIComponent(encodedUrl);

        if (!confirm('確定要刪除此筆資料？此操作無法復原。')) return;
        ...
        try {
            ...
            const res = await fetch(`/api/data/${apiType}?url=${encodeURIComponent(url)}`, { method: 'DELETE' });
            ...
            // 更新 DB Viewer 快取
            const cacheKey = apiType === 'trend' ? 'trends' : apiType;
            dbDataCache[cacheKey] = dbDataCache[cacheKey].filter(it => it.url !== url);
            // 更新計數器
            const countMap = { science: 'dbCountScience', social: 'dbCountSocial', trend: 'dbCountTrends' };
            const countEl = document.getElementById(countMap[apiType]);
            if (countEl) countEl.textContent = Math.max(0, parseInt(countEl.textContent || '0') - 1);

            // 同步移除左側頁籤中的對應卡片
            const mainPanelCard = document.querySelector(`.data-card[data-id="${CSS.escape(url)}"]`);
            if (mainPanelCard) {
                mainPanelCard.style.transition = 'opacity 0.3s, transform 0.3s';
                mainPanelCard.style.opacity = '0';
                mainPanelCard.style.transform = 'scale(0.95)';
                setTimeout(() => mainPanelCard.remove(), 300);
                // 若該卡片正被選中，清除選取狀態
                if (selectedState.science && selectedState.science.id === url) deselectCard('science');
                if (selectedState.spice && selectedState.spice.id === url) deselectCard('spice');
            }
            ...
            btnEl.innerHTML = '<i class="fa-solid fa-trash-can"></i>';
