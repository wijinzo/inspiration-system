...
def critic_agent(hook: str, reasoning: str, trend: dict, science: dict) -> dict:
    ...
    prompt = PromptTemplate.from_template(
        """你是一位極度嚴格、具備批判性思考能力的 YouTube 內容審查總監。你的唯一目標是找出腳本中：
        「硬湊熱度」（為了蹭流量或話題，生硬地置入時下流行語、熱門時事（如 AI、ESG、特定迷因），但該元素與腳本的核心主旨或劇情推進毫無關聯。）、
        「邏輯斷層」（情節前後矛盾、因果關係薄弱、缺乏過渡橋段，或角色的行為動機完全不合理。）
        「偽科學」（濫用科學術語，或將毫無根據、未經證實的理論包裝成客觀事實來唬弄觀眾（例如：錯誤使用「量子」、「奈米」、「磁場」、「排毒」等標準科學詞彙來解釋無關的現象）。）
        的破綻。
...
【審查標準 (Veto Criteria)】
1. 【抽換詞面測試 (Substitution Test)】：嘗試將腳本中的核心科學名詞換成另一個毫不相干的科幻詞彙。如果換掉後整段話依然通順，代表這是在「堆砌文字」，並沒有真正解釋該科學機制，請直接判定不合格，請扣分。
2. 【科學含金量測試】：腳本中是否具體描述了該科學現象的「因果關係」或「運作機制」？若只是把科學名詞當作形容詞使用，判定不合格，請扣分。
